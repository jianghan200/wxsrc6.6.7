package android.support.v7.widget;

import android.content.Context;
import android.content.res.TypedArray;
import android.graphics.drawable.Drawable;
import android.util.AttributeSet;

public final class ap
{
  public final TypedArray Ww;
  private final Context mContext;
  
  private ap(Context paramContext, TypedArray paramTypedArray)
  {
    this.mContext = paramContext;
    this.Ww = paramTypedArray;
  }
  
  public static ap a(Context paramContext, AttributeSet paramAttributeSet, int[] paramArrayOfInt)
  {
    return new ap(paramContext, paramContext.obtainStyledAttributes(paramAttributeSet, paramArrayOfInt));
  }
  
  public static ap a(Context paramContext, AttributeSet paramAttributeSet, int[] paramArrayOfInt, int paramInt)
  {
    return new ap(paramContext, paramContext.obtainStyledAttributes(paramAttributeSet, paramArrayOfInt, paramInt, 0));
  }
  
  public final Drawable bP(int paramInt)
  {
    if (this.Ww.hasValue(paramInt))
    {
      paramInt = this.Ww.getResourceId(paramInt, 0);
      if (paramInt != 0) {
        return h.eJ().a(this.mContext, paramInt, true);
      }
    }
    return null;
  }
  
  public final int bQ(int paramInt)
  {
    return this.Ww.getColor(paramInt, -1);
  }
  
  public final boolean getBoolean(int paramInt, boolean paramBoolean)
  {
    return this.Ww.getBoolean(paramInt, paramBoolean);
  }
  
  public final int getDimensionPixelOffset(int paramInt1, int paramInt2)
  {
    return this.Ww.getDimensionPixelOffset(paramInt1, paramInt2);
  }
  
  public final int getDimensionPixelSize(int paramInt1, int paramInt2)
  {
    return this.Ww.getDimensionPixelSize(paramInt1, paramInt2);
  }
  
  public final Drawable getDrawable(int paramInt)
  {
    if (this.Ww.hasValue(paramInt))
    {
      int i = this.Ww.getResourceId(paramInt, 0);
      if (i != 0) {
        return h.eJ().a(this.mContext, i, false);
      }
    }
    return this.Ww.getDrawable(paramInt);
  }
  
  public final int getInt(int paramInt1, int paramInt2)
  {
    return this.Ww.getInt(paramInt1, paramInt2);
  }
  
  public final int getLayoutDimension(int paramInt1, int paramInt2)
  {
    return this.Ww.getLayoutDimension(paramInt1, paramInt2);
  }
  
  public final int getResourceId(int paramInt1, int paramInt2)
  {
    return this.Ww.getResourceId(paramInt1, paramInt2);
  }
  
  public final CharSequence getText(int paramInt)
  {
    return this.Ww.getText(paramInt);
  }
  
  public final boolean hasValue(int paramInt)
  {
    return this.Ww.hasValue(paramInt);
  }
}


/* Location:              /Users/Han/Desktop/wxall/微信反编译/反编译 6.6.7/dex2jar-2.0/classes-dex2jar.jar!/android/support/v7/widget/ap.class
 * Java compiler version: 6 (50.0)
 * JD-Core Version:       0.7.1
 */