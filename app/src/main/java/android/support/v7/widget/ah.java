...
final class ah
{
  static int a(RecyclerView.q paramq, ab paramab, View paramView1, View paramView2, RecyclerView.h paramh, boolean paramBoolean)
  {
    ...
    if (!paramBoolean) {
      return Math.abs(RecyclerView.h.bd(paramView1) - RecyclerView.h.bd(paramView2)) + 1;
    }
    int i = paramab.aV(paramView2);
    int j = paramab.aU(paramView1);
    return Math.min(paramab.fu(), i - j);
  }
  ...
  static int a(RecyclerView.q paramq, ab paramab, View paramView1, View paramView2, RecyclerView.h paramh, boolean paramBoolean1, boolean paramBoolean2)
  {
    ...
    return j;
    label45:
    i = Math.min(RecyclerView.h.bd(paramView1), RecyclerView.h.bd(paramView2));
    j = Math.max(RecyclerView.h.bd(paramView1), RecyclerView.h.bd(paramView2));
    if (paramBoolean2) {}
    for (i = Math.max(0, paramq.getItemCount() - j - 1);; i = Math.max(0, i))
    {
      ...
      j = Math.abs(paramab.aV(paramView2) - paramab.aU(paramView1));
      int k = Math.abs(RecyclerView.h.bd(paramView1) - RecyclerView.h.bd(paramView2));
      float f = j / (k + 1);
      return Math.round(i * f + (paramab.fs() - paramab.aU(paramView1)));
    }
  }
...
/* Location:              /Users/Han/Desktop/wxall/微信反编译/反编译 6.6.7/dex2jar-2.0/classes3-dex2jar.jar!/android/support/v7/widget/ah.class
